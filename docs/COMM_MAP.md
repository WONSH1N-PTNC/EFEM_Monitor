...
> 출처: `docs/IO List_260801.xlsx` **(주 출처, 2026-08-10 반영)**, `docs/CH2.6.JKBLD300V2 User Manual_v1.1.pdf`, `docs/26.PSMC1-V1.5-KO_20250221_W.pdf`, `docs/DSE_통신 자료_260710.xlsx`, `docs/ESAM 운용방법 설명자료_260309 V1.4.6.pdf`
...
→ 출하 시 이미 38400 으로 설정되어 있는지 확인 요청. 아니라면 위 절차를 설치 매뉴얼에 포함해야 합니다.
...
> **⚠ 기본 보레이트 불일치.** 드라이버 RS-485 기본값은 115200 인데 CH2 는 38400 입니다. `0x4044 = 2` 설정 후 재기동이 필요합니다. 출하 시 설정 여부 확인 요청.

**PLC (LS XBM, ID 25)**

`0x000A` 한 워드에 디지털 입력 8점이 비트마스크로 들어옵니다. **1트랜잭션으로 전부 읽힙니다.**

| bit | PLC DATA | Tag | 내용 | 접점 |
|---|---|---|---|---|
| 0 | D10.1 | EMO1 | **비상정지** | NO, 정상 OFF |
| 1 | D10.2 | BF01 | EC BLDC 냉각팬 정지 | NPN |
| 2 | D10.3 | BF02 | EL BLDC 냉각팬 정지 | NPN |
| 3 | D10.4 | BF03 | ER BLDC 냉각팬 정지 | NPN |
| 4 | D10.5 | BF04 | SL BLDC 냉각팬 정지 | NPN |
| 5 | D10.6 | BF05 | SR BLDC 냉각팬 정지 | NPN |
| 6 | D10.7 | FAN01 | 컨트롤박스 팬 T 정지 | NPN |
| 7 | D10.8 | FAN02 | 컨트롤박스 팬 B 정지 | NPN |

> **⚠ 도어 열림 · 메인 차단기 입력이 없습니다.** 인터록 IL-03(차단기) · IL-05(도어)는 신호원이 없어 **비활성**입니다. SPARE DI 가 2점 남아 있으므로 배선 추가 시 활성화만 하면 됩니다. → Open Issue #7

> **⚠ 이전 초안의 EMO = D10.6 은 오류였습니다.** 실제 D10.6 은 "SR BLDC 냉각팬 정지"입니다. 그 매핑으로 배선했다면 비상정지를 냉각팬 알람으로 읽게 됩니다.
