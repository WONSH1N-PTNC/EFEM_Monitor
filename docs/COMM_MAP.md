...
| 스케일 팩터 및 부호 처리 | **TBD** — 잠정 0.1 Pa/LSB, Int16 |
...
잠정값 **0.1 Pa/LSB** 를 세 곳이 공유합니다.

| 위치 | 값 |
|---|---|
| `config/device-map.json` → `DiffPressure.pressurePa.scale` | 0.1 |
| `SimulatedPressureSensor.PaPerLsb` | 0.1 |
| `ServicesIntegrationTests.CreateSensorType()` | 위 상수를 참조 |

**1 Pa/LSB 는 쓸 수 없습니다.** 센서 1 대역이 4~8 Pa 이므로 유효 분해능이 4 LSB 가 되어 밴드 제어가 성립하지 않습니다. ±2000 Pa 를 0.1 Pa/LSB 로 표현하면 ±20000 이고 Int16 범위(±32767) 안에 들어갑니다.

매뉴얼을 확보하면 `device-map.json` 과 `PaPerLsb` 를 **함께** 바꿔야 합니다. 한쪽만 바꾸면 `배포_설정의_압력_스케일이_시뮬레이션과_일치한다` 가 막습니다.

어긋난 상태가 위험한 이유는 조용하기 때문입니다. 측정값이 배수만큼 틀려도 제어·화면·알람·상위 보고가 모두 같은 값을 쓰므로 **각 계층은 자기 기준으로 일관**하고, 어디에서도 오류가 나지 않습니다. 실장비에서 처음 드러납니다.
