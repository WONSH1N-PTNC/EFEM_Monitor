...
- 알람 66종(`Alarm LIST` 시트) + 사내 진단 8종 + 인터록 5종
...
> **2026-08-11 갱신.** 초기 31종(운용 PDF p.9)에서 `ESAM_IO List_260806` 의 `Alarm LIST` 시트
> **66종**으로 확정했습니다. 코드는 시트 No. 를 그대로 씁니다(`AL-01`~`AL-66`).
> 사내 진단 알람 8종은 `DG-` 범위로 분리해 상위(GEM)에 보고하지 않습니다.
> 압력 26종은 임계값을 갖지 않고 `recipe.json` 에서 가져옵니다(CONFIG_MODEL.md §9).
...
- `BlockingCollection<SystemSnapshot>` → 500ms 또는 100건 단위 **단일 트랜잭션 배치 insert**
  (구현: `src/Esam.Services/DataLogger.cs`)
  - **큐에 넣는 경로는 절대 막히지 않는다.** `SnapshotPublished` 는 포트 워커 스레드에서
    발생한다. 거기서 대기하면 폴링 주기가 디스크 사정에 묶이고, 기록이 제어를 멈추게 한다.
    `TryAdd(snapshot, 0)` — 대기 시간 0 이 이 설계에서 가장 중요한 인자다.
  - **큐는 유한하다**(기본 2000행 ≒ 200ms 주기로 400초분). 무한 큐를 두면 디스크가
    멈췄을 때 메모리가 대신 차오르다가 프로그램이 죽는다. 죽는 것은 제어가 멈추는 것이다.
  - 넘치면 **새 행을 버리고 버린 수를 센다.** 세지 않으면 트렌드의 구멍을
    "아무 일도 없던 시간" 으로 읽게 된다. 처음 버린 시점에 `LOG-02` 로 한 번 알린다.
  - **스냅샷을 큐에 넣고 변환은 적재 스레드가 한다.** 트렌드 행으로 옮기는 일(사전 조회
    23회)을 워커 스레드에서 하지 않기 위한 배치다. `SystemSnapshot` 은 불변이라 안전하다.
  - 배치 마감은 `Stopwatch` 로 잰다. 시각을 뺄셈하면 운전 중 시스템 시각이 바뀔 때
    (NTP 보정·수동 변경) 마감이 과거나 먼 미래가 되어 배치가 즉시 끊기거나 안 끊긴다.
  - **연속 5회 실패하면 기록을 중단하고 `LOG-01`(Advisory)을 올린다.** 무한 재시도는
    배치마다 예외를 던지며 CPU 를 태우고 같은 경고로 화면을 덮는다.
  - 시작·종료 순서: 기록기를 **폴링보다 먼저 시작**하고 **폴링보다 나중에 멈춘다.**
    먼저 멈추면 파킹 구간 — 밸브를 닫고 팬을 세운 마지막 몇 초 — 이 빠진다.
    그 구간은 사고 분석에서 가장 먼저 보는 곳이다.
- **스위치가 둘이다.** `RuntimeOptions.EnableDataLogging`(조립, 기본 `false`)은 "이 프로세스가
  기록을 담당하는가", `control.json` 의 `logging.enabled`(운용, 배포 `true`)는 "현장에서 기록을
  켤 것인가". 조립 스위치를 기본 `false` 로 둔 이유는 런타임을 조립하는 테스트가 수백 건이라,
  켜 두면 그 전부가 작업 폴더에 DB 파일을 만들기 때문이다. 켜는 결정은 `HmiHost` 한 곳에서만 한다.
...
| 3 | **Config** ✅ | S1-1~1-3 / S2-1~2-5 / S3-1~3-5의 **Set · 하한 · 상한** 테이블(`recipe.json`), 통신 포트·전송 방식·통로 활성화 | Engineer **→ S7 구현.** 이탈 확정 시간(Time)과 FFU·Particle·Temp/Humidity Limit 은 `alarms.json` 소관이라 **알람 설정 화면(S7.5)** 으로 분리 |
| ~~4~~ | **I/O (Status)** ✅ | ①FFU ②송풍팬 ③스로틀밸브 ④차압센서 ⑤FDC ⑥쿨링팬 ⑦Temp(CtrlBox) ⑧Temp(EFEM) ⑨Humidity ⑩Particle ⑪MFC ⑫풍속 — **연결/정상동작 상태 램프**, PLC 디지털 입력 D10.0~D10.8 | Maintenance **→ S7.5-a 구현.** 램프는 5상태(정상/열화/무응답/사용 안 함/구성 없음)에 **미구현**을 더해 6종. 압력 원시 레지스터와 PLC 설정 극성을 함께 표시해 커미셔닝 2건을 판정한다. 쓰기가 없어 잠금 관문 대상이 아니다 |
...
| ~~8~~ | **Alarm (Config)** ✅ | 알람 74종의 `threshold`·`debounceMs`·`severity`·`resetPolicy`·`enabled` 편집. 코드·판정 대상·조건은 읽기 전용 | Engineer **→ S7.5-b 구현.** 압력 26룰의 임계값 칸은 `recipe.json` 소관이라 잠근다. 치명 알람 비활성화는 별도 확인을 거친다 |
...
| **C3** ✅ | 알람 66종 재작성 — 임계값을 `recipe.json` 에서 가져오고, 경로 해석 검증을 신설 | 2026-08-11 완료. **테스트 398건 전량 통과** |
| **S6** | 밸브·팬 드라이버 실장비 검증 | 실 밸브 1대 위치 정확도, 인터록 응답시간 실측 |
| **S7** ✅ | **HMI 실연결 — Operate + Config 2화면.** 난수 시뮬레이터 제거 후 `DataStore` 폴링, 구성 경고 배너, 레시피 편집, 통신·통로 설정, 쓰기 잠금 관문 | 2026-08-11 완료. **테스트 439건.** 화면을 실제로 띄워 결함 4건 발견(11.3) |
| **S7.5-a** ✅ | **I/O Status 화면** + 디바이스별 통신 건강 상태(`DeviceHealth`) + 결함 2건(D19·D20) | 2026-08-12 완료. 화면을 띄워 램프 12종·DI 8점·압력 13ch 확인 |
| **S7.5-b** ✅ | **알람 설정 화면** — 74룰의 임계값·확정 시간·심각도·해제정책·활성 여부 편집 | 2026-08-12 완료. **테스트 512건 전량 통과.** 주석 보존 부분 수정 저장, 발생 상태 승계 교체 |
| **C5** ✅ | **`control.json` 신설** — 제어 파라미터·통로 구성의 배포 파일화 + 결함 1건(D22) | 2026-08-19 완료. 파일 값 ↔ 코드 기본값 대조 |
| **S7.5-c** ✅ | **Maintenance — 영점 교정 · 수동 조작 · Homing** | 2026-08-19 완료. **테스트 564건 전량 통과.** 관문 3겹 + 화면 이탈 시 자동 파킹. 화면을 띄워 결함 1건 발견(D23) |
| **S8** | 데이터 로깅 + CSV Export | 24시간 연속 적재, DB 크기/조회 성능 |
| **S9** | 권한/감사로그 (알람 규칙 집합은 C3 에서 완료) | 알람 66종 전수 유발 테스트 |
...
| `Advisory` | 일부 계측이 빠짐, 기록이 밀리거나 멈춤 | `CFG-ADDR` 주소 미확정, `ALM-02` 개별 알람 비활성, `SIM-01` 시뮬레이션 슬레이브 없음, `LOG-01` 기록 중단, `LOG-02` 적재 지연으로 버림, `LOG-03` 오래된 파일 삭제 실패, `LOG-04` 기록 시작 실패 |

`LOG-*` 가 모두 `Advisory` 인 것은 의도입니다. **기록은 통보 기능**이므로, 기록이 안 된다는 이유로 압력 제어를 세울 근거가 없습니다. 반대로 조용히 실패하지도 않습니다 — 버린 건수와 중단 사유가 배너에 남습니다.
...
### 11.3 S7 — 화면을 띄워 보고 찾은 결함 4건 (2026-08-11 완료)

S7 은 화면과 런타임을 잇는 작업입니다. 로직을 새로 만들지 않았으므로 결함이 나올 자리가 없다고
보았는데, **네 건이 나왔고 셋은 테스트로 잡히지 않는 종류였습니다.**

| # | 결함 | 실패 시나리오 | 심각도 |
|---|---|---|---|
| **D15** | **시뮬레이션에 PLC 슬레이브가 없다.** 안전 입력(D10.x)이 무응답이 되어 IL-04 가 즉시 발동한다 | 프로그램을 띄우면 영구 SafeStop. 운전 자체가 불가능하다. 더 나쁜 것은 **안전 입력 경로가 시뮬레이션에서 한 번도 실행된 적이 없었다**는 사실이다 | **높음** |
| **D16** | **플랜트 시계가 전진하지 않는다.** `AdvancePlantInRealTime` 이 꺼져 있어 밸브·팬 응답이 정지 상태 | 원점 복귀가 끝나지 않아 AUTO 버튼이 영구 비활성. 사유가 화면에 없어 "버튼이 고장" 으로 보인다. 함께 `PlantModel` 의 다중 스레드 경합도 드러났다 | **높음** |
| **D17** | **없는 COM 포트 이름이 프로그램을 죽인다.** `ModbusPortWorker.Start` 가 `_transport.Open()` 을 그대로 호출 | **가장 흔한 커미셔닝 실수에 가장 나쁜 반응.** 창이 뜨지 못하므로 원인을 볼 수도, 설정을 고칠 수도 없다. 설정 파일을 편집기로 직접 열어야 복구된다 | **높음** |
| **D18** | **설정 검증 실패 시 "되돌린다" 는 주석이 거짓.** 살아 있는 `DeviceMap` 을 먼저 고친 뒤 검증하고, 실패하면 `Load()` 로 그 맵을 다시 읽었다 | 파일은 옛 값, 메모리는 새 값으로 남는다. 게다가 작업자가 입력한 값까지 지워 **무엇을 고치려 했는지도 사라진다.** ViewModel 테스트를 쓰는 중에 발견 | 중간 |

**D15·D16·D17 은 통합 테스트 385건을 전부 통과한 상태에서 나왔습니다.** 테스트는 자체 `CreateMap()` 을
쓰고, 시간을 직접 통제하고, 단일 스레드로 돕니다. 셋 다 그 전제가 실제 실행과 다른 지점에서 터졌습니다.

> **DESIGN.md 는 "가상 플랜트 덕분에 실장비 없이 안전 결함을 찾았다" 고 적어 두었습니다.**
> 절반만 맞았습니다. 가상 플랜트에 **PLC 가 없었으므로 안전 입력 경로는 검증된 적이 없었습니다.**
> 시뮬레이션이 실제와 다른 부분은 그 자체로 검증 공백입니다.

D17 의 대책은 예외를 잡는 것에서 끝나지 않습니다. 포트 열기 실패를 `COM-01` **차단 구성 경고**로
올려 배너에 남기고, 열린 포트는 그대로 기동하며, 폴링 루프가 매 사이클 재개방을 시도합니다.
변환기 전원이 돌아왔을 때 프로그램을 재시작해야 한다면 현장에서 그 프로그램을 신뢰하지 않게 됩니다.

D18 의 대책은 **살아 있는 맵을 아예 만지지 않는 것**입니다. 사본에 반영해 검증하고, 성공하면
파일에 쓴 뒤 재조립이 그 파일을 다시 읽습니다. 되돌릴 것을 만들지 않는 편이 되돌리는 코드보다 낫습니다.

### 11.5 S7.5-b — 알람 설정을 편집 가능하게 만들며 정한 것 (2026-08-12 완료)

알람 74종에는 편집 수단이 없었다. 현장에서 알람이 과민하면 `alarms.json` 을
편집기로 직접 열어야 했고, **그 경로는 저장 검증을 거치지 않는다.**
쉼표 하나를 잘못 찍으면 다음 기동에서 장비가 뜨지 않는다.

#### 왜 역직렬화해서 다시 쓰지 않는가

`alarms.json` 에는 **비활성 규칙의 사유**를 적은 한글 주석이 200여 줄 있다.
"동작하는 줄 알았는데 아니었다" 를 막으려고 남긴 설명이다.
규칙 하나를 고치려고 파일을 통째로 다시 쓰면 그 설명이 전부 사라지고,
정렬과 구획선도 함께 사라진다. **다음 사람은 알람이 왜 꺼져 있는지 알 수 없게 된다.**

Json.NET 의 `CommentHandling.Load` 는 주석 *내용*은 살리지만 다시 쓸 때
줄 주석(`//`)을 블록 주석으로 바꾸고 파일 전체를 재정렬한다. 그래서 쓰지 않는다.

대신 `AlarmDocumentEditor` 가 원문을 훑어 **바꿀 값의 위치만** 찾고 그 구간을 치환한다.
손대지 않은 바이트는 하나도 변하지 않는다. 표기가 다르지만 같은 수(`-100.0` vs `-100`)는
건드리지 않는다 — 그러지 않으면 아무것도 편집하지 않고 저장한 사람이 74줄짜리 변경을 만든다.

#### 왜 런타임을 재조립하지 않는가

`AlarmService` 는 생성자에서만 규칙을 받았다. 규칙을 바꾸려면 런타임 전체를
재조립하는 수밖에 없었고, 그러면 워커가 멈추고 포트를 다시 열고 밸브가 원점 복귀를
다시 한다. **운전 중에 임계값 하나를 고칠 수 없다는 뜻이었다.**

`AlarmEvaluator.ReplaceRules()` 는 규칙 목록과 상태 사전을 락 안에서 통째로 바꾼다.
제자리에서 고치면 판정 중인 스레드가 절반만 갱신된 목록을 읽어,
규칙 A 는 새 임계값·규칙 B 는 옛 임계값으로 판정되는 순간이 생긴다.

**같은 코드의 발생 상태는 승계한다.** 승계하지 않으면 화면에서 다른 알람의 확정 시간을
고쳤다는 이유로 **떠 있던 Manual 정책 알람이 전부 사라진다.** 저장 버튼이 Reset 을
대신하는 셈이고, 현장에서는 "알람이 저절로 꺼졌다" 로 보인다.
사라지는 코드 중 발생 중이던 것은 결과로 돌려 화면에 적는다.

#### 편집 범위를 좁힌 이유

| 항목 | 편집 | 근거 |
|---|---|---|
| `threshold`·`debounceMs`·`severity`·`resetPolicy`·`enabled` | 가능 | 현장 조정 대상 |
| `code`·`source`·`condition` | **불가** | 구조다. 화면에서 바꾸면 경로 해석 검증을 통과하지 못하는 조합이 나온다. 그것은 알람 편집이 아니라 알람 재정의다(SCREEN 08 은 S9) |
| 압력 26룰의 `threshold` | **잠금** | `recipe.json` 이 관리한다. 여기서 고칠 수 있게 하면 같은 숫자가 두 곳에 살고, 화면만 보고는 어느 쪽이 적용되는지 알 수 없다 |

치명(Critical) 27건의 비활성화는 **막지 않되 한 번 더 묻는다.** 막으면 파일을 직접
열게 되고, 그 경로에는 검증도 확인 절차도 없다. 저장에 성공하면 기준선을 옮겨
같은 해제를 다시 묻지 않는다 — 두 번째부터 형식이 된 확인은 아무도 읽지 않는다.

---

### 11.9 D23 — 기동할 때마다 Fault 로 갇혔다 (2026-08-19 완료)

정비 화면이 단계를 조작 거부 사유로 찍으면서 드러났다. 증상은 오래된 것이다 —
**"AUTO 버튼이 안 눌린다."** D16 때 같은 증상을 플랜트 시계 문제로 진단했는데,
**원인이 하나 더 있었다.**
...
| | |
|---|---|
| 1 | `device-map.json` 에 PLC-1 이 있으므로 `SafetyInputsConfigured = true` |
| 2 | IL-04(안전 입력 신뢰 불가)는 `Plc.Quality != Good` 이면 발동 |
| 3 | 인터록 판정은 **포트마다 폴링 사이클이 끝날 때** 돈다. CH2(10 tx, 113 ms)가 CH1(14 tx, 218 ms, **PLC 가 여기 있다**)보다 먼저 끝난다 |
| 4 | CH2 의 첫 사이클 시점에 PLC 는 아직 `NoData` → IL-04 발동 → **SafeStop** |
| 5 | 곧 CH1 이 끝나 PLC 가 `Good` → 해소 → `SafeStopCleared` → **Fault** |

5번 전이는 의도된 설계다 — *"물리 안전장치가 동작한 뒤에는 밸브 위치를 다시
확인하고 시작해야 한다."* 문제는 **거기서 나올 수가 없었다**는 것이다.

#### 나올 수 없었던 이유

상태머신은 `Fault → ResetRequested → Init` 전이를 정의해 두었다.
그런데 **`ResetRequested` 를 발생시키는 코드가 프로덕션에 하나도 없었다.**
테스트에만 있었다.

배너의 "장애 해제" 는 `SafeStopCleared` 를 쏴서 **SafeStop 을 Fault 로 내려보내는**
명령이고, 그 버튼은 `Phase == SafeStop` 일 때만 보인다. **도착지에서 사라진다.**

> 두 결함이 겹쳐 있었다. 하나는 **매번 Fault 로 들어가게 했고**,
> 다른 하나는 **나올 수 없게 했다.** 각각만 있었다면 드러났을 것이다 —
> 들어가기만 했다면 재시작으로 넘어갔을 것이고, 나올 수 없기만 했다면
> 애초에 들어갈 일이 없었다.

#### 대책 1 — "아직 안 왔다" 와 "오다가 끊겼다" 를 구분한다

`ControlConfig.SafetyInputGraceMs`(기본 5000 ms)를 두고, **첫 수신 전**에는 `NoData` 를
보류한다. 다음 두 가지는 유예하지 않는다.

- **한 번이라도 수신한 뒤의 품질 저하** — 기동 지연이 아니라 통신 상실이다
- **`Bad`·`Stale`·`Uncertain`** — 응답이 오기는 왔는데 믿을 수 없다는 뜻이다

**유예가 지나도록 한 번도 수신하지 못하면 발동한다.** "NoData 는 봐준다" 로 두면
PLC 가 영영 응답하지 않는 구성에서 IL-04 가 절대 발동하지 않는다.
안전 입력 없이 운전에 들어가는 것이 오탐보다 훨씬 나쁘다.

#### 대책 2 — Fault 에서 나올 길

`EsamRuntime.RequestRestart()` 가 `ResetRequested` 를 쏘고, 배너에 버튼을 노출한다.
**Ready 로 바로 가지 않는다.** Init 부터 다시 시작해 원점 복귀를 거친다.
재시작 전에 액추에이터를 파킹한다 — 장애 단계에서 밸브가 어디에 서 있는지 알 수 없고,
그 상태로 원점 복귀를 시작하면 예상치 못한 방향으로 움직인다.

**쓰기 잠금을 이 버튼에 걸지 않는다.** 잠긴 상태로 Fault 에 들어가면 잠금을 풀
화면(Settings)도 조작할 수 없어 복구 자체가 막힌다.

경고가 하나도 없어도 Fault 면 배너가 보이고, 접히지 않는다.

#### 곁가지 — 읽히지 않던 설정 키

C5 에서 `control.json` 에 넣은 `safetyInputsConfigured` 는 조립 루트가
device-map 기준으로 덮어쓰므로 **읽히지 않는 값**이었다. 고쳐도 아무 일이 없다.
제거하고 그 자리에 사유를 주석으로 남겼다.

> **C5 의 "값이 코드 기본값과 일치하는가" 대조가 이 종류를 놓쳤다.**
> 비교 목록에 없던 필드는 검증되지 않는다. `SafetyInputGraceMs` 를 목록에 넣었다.

---

### 11.8 S7.5-c — 액추에이터를 직접 움직이는 첫 화면 (2026-08-19 완료)

지금까지의 쓰기는 전부 설정 파일이었다. 여기는 **밸브와 팬을 직접 움직인다.**
성질이 다르므로 관문도 다르다.
...
| 겹 | 판정 | 근거 |
|---|---|---|
| 1 | 쓰기 잠금 | 레시피·알람 화면과 동일. 기본값이 거부다 |
| 2 | 단계 | **자동 운전 중 불가** — 제어 루프가 다음 주기에 다른 값을 쓴다. 사람이 넣은 값은 눈 깜짝할 사이 사라지고, **조작이 먹은 것처럼 보이다가 원인 없이 되돌아간다.** **원점 복귀 전 불가** — 밸브 위치를 신뢰할 수 없는 상태에서 개도율을 지정하는 것은 의미가 없다 |
| 3 | 인터록 | 발동 중에는 불가. 지령 우선순위가 `Automatic(0) < Manual(1) < Interlock(2)` 이라 **인터록이 항상 수동을 이긴다** |

판정은 `EsamRuntime.DescribeManualDenial()` **한 곳**에 있다. 화면은 그 문자열을
그대로 띄운다. 이유 없이 눌리지 않는 버튼은 프로그램이 멈춘 것처럼 보이고,
현장에서는 그때부터 화면을 믿지 않는다.

#### 화면을 떠나면 파킹한다

밸브를 60 % 로 열어 두고 다른 화면으로 넘어가면 그 상태가 그대로 남는데,
다른 화면에는 그 사실이 보이지 않는다. 화면 전환과 창 종료 두 경로 모두에서
`ParkManual()` 을 부른다.

**인터록 파킹(`ControlEngine.ParkActuators`)과 다르다.** 그쪽은 인터록
우선순위로 보내 어떤 지령도 이기지만, 이쪽은 수동 우선순위다.
안전 사건이 아니라 **사람이 하던 일을 정리하는 것**이므로 인터록인 척하지 않는다.

그리고 **파킹 지령을 보낸 것과 액추에이터가 실제로 닫힌 것은 다르다.**
통신이 끊겨 있으면 지령은 큐에 남는다. 그래서 수동 조작이 살아 있는 동안
배너에 상시 표시한다 — 구성 경고와 달리 접히지 않는다.
접을 수 있는 것은 "인지했다" 로 끝나는 사실이고, 이것은 **지금 이 순간의 운전 상태**다.

#### 영점 교정

`RawPa`(보정 전 값) 20회 평균을 새 영점으로 **제안**한다. 곧바로 덮어쓰지 않는다 —
대기압이 아닌 상태에서 잡은 영점은 이후 모든 측정에 오차를 싣고,
**그 사실은 한참 뒤에야 드러난다.** 제안값·현재값·차이를 나란히 두고,
적용한 뒤에도 직전 값으로 되돌릴 수 있다.

품질이 나쁜 표본은 세지 않는다. 다만 통신이 끊긴 센서 하나 때문에 화면이
영원히 "수집 중" 으로 남지 않도록 3배 시간에서 끊고 그 사실을 남긴다.

저장은 `DeviceMapDocumentEditor` 를 그대로 쓴다(D21 에서 만든 경로).
어느 포트도 갖고 있지 않은 센서 ID 는 따로 돌려받아 화면에 적는다 —
조용히 넘어가면 **영점을 잡았는데 값이 그대로인** 상태가 된다.

---

### 11.7 C5 — `control.json` 신설 (2026-08-19 완료)

S7.5-c(Maintenance)의 "제어 파라미터 실시간 조정" 을 설계하다가 드러났다.
**바꾼 값을 쓸 곳이 없었다.**

`HmiHost.LoadControl()` 은 `ControlConfig` 를 코드 기본값으로 만들고 체인만 조립했다.
주석에도 그렇게 적혀 있었다 — *"`control.json` 은 아직 배포 파일이 없다"*.
즉 **Step·Dwell·밸브 최대 개도·팬 최소 회전수가 전부 하드코딩**이었고,
현장에서 하나를 조정하려면 재컴파일이 필요했다. **재컴파일이 필요하면 아무도 조정하지 않는다.**

#### D22 — 통로 활성화가 저장되지 않았다

`SettingsViewModel.ApplyChains()` 는 `chain.Enabled` 를 **메모리에만** 반영했다.
프로그램을 다시 띄우면 꺼 둔 통로가 전부 켜진 상태로 돌아왔고,
화면에는 그 사실이 드러나지 않았다. **정비 중 통로를 꺼 두고 퇴근하면
다음 기동에서 그대로 돈다.**

#### 파일이 없어도 기동한다

제어 파라미터는 코드 기본값이 있고 그 값으로도 운전은 성립한다. 여기서
실패시키면 설정 파일 하나가 없다는 이유로 장비가 뜨지 않는다.
다만 **조용히 넘어가지 않는다** — 파일이 없거나 읽지 못하면 `CFG-CTL`
구성 경고로 배너에 남긴다. 현장에서 값을 고쳤는데 반영되지 않는 상태가
가장 나쁘고, 그 원인이 대개 "프로그램이 다른 폴더를 보고 있다" 이다.

이를 위해 `RuntimeOptions.AdditionalWarnings` 를 신설했다. 설정 파일을
런타임 밖에서 읽는 경우의 경고를 배너까지 잇는 통로다.

> **C5 는 값을 옮기는 작업이지 바꾸는 작업이 아니다.** 파일을 신설하면서 값이
> 달라지면 그 순간부터 설비가 다르게 움직인다. 그래서 배포 파일의 값이
> 코드 기본값과 일치하는지 대조하는 테스트를 함께 둔다.

---

### 11.6 D21 — 설정 저장이 주석을 지우고 있었다 (2026-08-12 완료)

S7.5-b 에서 `alarms.json` 만 부분 수정으로 바꿨는데, **나머지 두 파일은 여전히 전체
재직렬화**였다. Settings·Recipe 화면은 S7 에서 만들었고 **아무도 아직 현장에서
저장해 본 적이 없어서** 드러나지 않았다.

| 파일 | 주석 | 종전 저장 경로 | 결과 |
|---|---|---|---|
| `alarms.json` | 55줄 | `AlarmDocumentEditor` | 보존 |
| `device-map.json` | 55줄 | `CommunicationConfigLoader.ToJson` | **전부 소실** |
| `recipe.json` | 29줄 (52줄 중) | `RecipeConfigLoader.ToJson` | **전부 소실** |

**무엇이 사라지는가.** `device-map.json:64` 에 커미셔닝 미확정 항목의 근거가 있다.

```
// 0.1 Pa/LSB. 매뉴얼 미확보로 잠정값이다(COMM_MAP.md §7 TBD).
// 1 Pa/LSB 로는 센서1 대역(4~8 Pa)이 4 LSB 로 양자화되어 제어가 성립하지 않는다.
// 시뮬레이션 슬레이브도 같은 값을 쓴다. 한쪽만 바꾸면 테스트가 막는다.
```

현장에서 **COM 포트를 한 번 바꾸면** 이 세 줄이 사라진다. 그 뒤에 스케일을 확정하러
온 사람은 0.1 이 잠정값이라는 것도, 시뮬레이션과 짝이라는 것도 모른다.
`recipe.json` 은 52줄 중 29줄이 주석이라 **파일의 절반 이상**이 날아간다.

**대책은 스캐너의 일반화다.** `AlarmDocumentEditor` 안에 있던 스캐너를
`JsonTextScanner`·`JsonTextPatch` 로 꺼내 세 파일이 같은 경로를 쓴다.
파일마다 스캐너를 따로 두면 한쪽만 고쳐진 채로 남는다.
파일별 편집기는 "어느 값을 바꿀지" 만 안다.

> **테스트가 놓친 지점이 D19 와 같다.** 기존 테스트는 "검증 실패 시 파일이 바뀌지
> 않는가" 는 확인했지만 **성공했을 때 무엇이 남는가** 는 보지 않았다.
> 이제 무편집 저장 시 **바이트가 동일한가**를 세 파일 모두에서 확인한다.

---

### 11.4 S7.5-a — I/O 화면을 붙이며 찾은 결함 2건 (2026-08-12 완료)

I/O Status 화면은 값을 새로 만들지 않는다. 이미 있는 것을 표에 옮기는 작업이다.
그런데 **옮길 값을 찾다가 두 건이 나왔고, 둘 다 "화면이 조용히 거짓말을 하던" 종류다.**

| # | 결함 | 실패 시나리오 | 심각도 |
|---|---|---|---|
| **D19** | **PLC 측정점 키가 코드와 설정에서 다르다.** `SnapshotBuilder` 는 `di.fanStop0`·`temp.fan0` 을 찾고, `device-map.json` 은 `di.fanStop.0`·`temp.fan.0` 을 선언한다. **점 하나** | 조회가 항상 실패해 `FanStopAlarms` 는 영원히 false, `FanTemperatures` 는 영원히 null. **송풍팬 정지·과열 알람 10건(AL-19·22·25·28·31·32~36)이 절대 울리지 않는다.** 알람 목록 화면에는 전부 "활성" 으로 보인다 | **높음** |
| **D20** | **대시보드 우측 패널이 전부 고정값.** 풍속 3ch·온습도·판넬 온도·MFC·포트 진단이 `BuildStructure()` 에서 문자열로 주입된 뒤 갱신되지 않는다 | 실제 압력값 옆에 프로토타입 숫자가 나란히 놓인다. 통신이 끊겨도 값이 그대로라 **"멈췄다" 는 신호조차 없다.** 포트 이름(`BUS_A/B/C`)은 현재 2포트 구성과 다르고, `218 ms` 는 S3 시뮬레이션 실측을 적어 둔 것이다. **S6 에서 폴링 예산을 이 숫자로 확인하면 입력한 적 없는 값을 실측으로 읽는다** | **높음** |

**D19 를 441건이 놓친 이유**는 D15~D17 과 같다. `ServicesIntegrationTests.CreateMapWithPlc()` 가
**코드와 같은 방식으로** 키를 만든다. 양쪽이 같은 오타를 공유하면 어긋난 사실이 드러나지 않는다.
그래서 대책은 수정이 아니라 **대조**다.

1. `PointKeys` 가 접두 상수 대신 `DiFanStop(int)`·`TempFan(int)`·`TempFault(int)` 를 노출한다.
   조립 규칙이 호출부에 있으면 규칙이 호출부 수만큼 생긴다.
2. `PlcPointContractTests` 가 **배포되는 `device-map.json`** 을 읽어, 조회하는 키가 전부 선언되어
   있는지 / 선언된 키를 전부 소비하는지 양방향으로 대조한다.
   문자열 계약은 컴파일러가 잡아 주지 못한다. 잡아 줄 수 있는 것이 이 대조뿐이다.

> `temp.fault.0~4`(열전대 단선)는 선언되어 있으나 **아직 소비하는 코드가 없다.**
> 열전대가 끊기면 온도가 0 ℃ 로 읽히는데 지금은 그것을 정상값과 구분하지 못한다.
> 계약 테스트의 허용 목록에 명시해 두었으므로, 소비를 시작하면 목록에서 빼면 된다.

**D20 의 대책은 디자인타임과 런타임의 경로를 나누는 것**이다. 예시 값은
`SeedDesignTimeReadouts()` 안에만 있고 `_runtime == null` 일 때만 호출된다.
값이 없으면 `- -` 로 보인다. 장치가 붙기 전에는 화면이 비어 보이겠지만 그것이 사실이다.

#### 함께 정리한 것

- `AuxiliaryReadings.Quality` 에 `Quality.Good` 을 그대로 적어 넣고 있었다.
  **온습도·풍속이 전부 끊겨도 "정상" 으로 보고되는 값이었다.** 읽는 코드가 없어 사고가
  나지 않았을 뿐이다. 보조 계측 장치 중 가장 나쁜 품질을 따르도록 고쳤다.
- `PressureReading.RawRegister` 는 통신 계층에서 채워지지 않아 **항상 0** 이다.
  I/O 화면은 스케일 역산으로 레지스터를 표시한다. 값을 채우거나 필드를 없애는 것은
  별도 항목으로 남는다.

---

#### ViewModel 을 테스트 대상에 넣은 이유

`Esam.Tests` 가 `Esam.Hmi` 를 참조합니다. 화면 코드를 테스트에서 참조하는 것이 일반적이지는 않지만,
이 프로젝트의 ViewModel 은 표시 로직이 아니라 **판단**을 들고 있습니다.

1. **입력 문자열 파싱** — 문화권을 고정하지 않으면 쉼표를 소수 구분자로 쓰는 지역 설정에서
   `6.5` 가 `65` 로 읽힙니다. 설정값이 10배가 된 채 아무 오류 없이 운전에 들어갑니다.
2. **쓰기 잠금 관문** — 기본값이 거부여야 합니다. 허용으로 두면 관문이 한 번도 동작하지 않은 채 출하됩니다.
3. **저장 전 검증 경유** — 화면에서 따로 검사하면 규칙이 두 곳에 생기고, 한쪽만 바뀌었을 때
   화면은 통과시키는데 다음 기동에서 로드가 실패해 장비가 뜨지 않습니다.

`Window`·`UserControl`·XAML 은 대상이 아닙니다. 런타임은 조립하되 기동하지 않으므로(워커 스레드 없음)
설정 적용 경로의 재조립까지 실시간 대기 없이 검증됩니다. 36건을 추가했습니다.
...
S1~S7 이 끝났습니다. 시뮬레이션 위에서 **폴링 → 스냅샷 → 제어 → 인터록 → 화면 → 설정 편집**
전 경로가 돌고, 설정값을 사람이 화면에서 고칠 수 있습니다.

우선순위 순으로:

1. **C4 — `gem-map.json` + `recipe:` 경로 스킴** (CONFIG_MODEL 4단계).
   상위 GEM 이 ECID 로 설정값을 읽고 쓰는 경로입니다. GEM 화면과 함께 진행합니다.
2. **S8 — 데이터 로깅.** SQLite 적재 + 트렌드 조회 + CSV Export.
   Data Log 화면과 고속 로거가 여기 묶입니다(Viewer 방식은 협의 항목 #8).
   화면은 I/O Status(**S7.5-a**), 알람 설정(**S7.5-b**), Maintenance(**S7.5-c**) 를 마쳤습니다.
   제어 파라미터 실시간 조정은 `control.json` 이 생겼으므로 이제 가능합니다.
3. **S6 — 실장비 검증.** 아래 항목이 커미셔닝 전에 확정되어야 합니다.

| 미확정 항목 | 확정되지 않으면 |
|---|---|
| 압력 스케일 0.1 Pa/LSB | 모든 압력값이 10배 틀어진다. COMM_MAP §7.1 의 3곳을 함께 고쳐야 한다 |
| PLC 입력 비트 극성 | 안전 입력이 반대로 읽힌다. EMO 를 누르지 않은 상태가 EMO 로 판정될 수 있다 |
| IO List `Driver` 시트 `0x4007` → `0x4006` | 밸브 지령이 엉뚱한 레지스터로 간다 |
| `SVID` 시트 24~28 "Front" → "Bottom" | GEM 변수 명칭 불일치 |
| 블로워 보레이트 115200 → 38400 | CH2 통신이 성립하지 않는다 |

> **S6 는 화면 없이 할 수 없는 작업이었습니다.** 위 5건은 모두 값을 눈으로 보며 확인해야 하고,
> 그 수단이 S7·S7.5-a 에서 생겼습니다. 압력 스케일은 I/O 화면의 **레지스터 열**을 센서 표시기와
> 대조해, PLC 비트 극성은 **설정 극성과 판정값을 나란히** 놓고 확정합니다.
>
> 통신 진단(포트별 사이클타임·성공률)은 **S7.5-a 전까지 하드코딩된 상수였습니다(D20).**
> 이 문장이 사실이 된 것은 2026-08-12 부터입니다.
