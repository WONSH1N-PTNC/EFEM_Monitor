...
| **S7.5** | 잔여 화면 — I/O Status, Maintenance, Data Log | 8시간 연속 UI 응답성/메모리 누수 |
...
2. **S7.5 — 잔여 화면.** I/O Status(램프 12종 + DI 8점), 알람 설정, Maintenance, Data Log.
   `alarms.json` 의 임계값·확정 시간은 아직 편집 수단이 없어 파일을 직접 고쳐야 합니다.
...
> 그 수단이 S7 에서 생겼습니다. 통신 진단(포트별 사이클타임·성공률)이 대시보드에 상시 노출됩니다.
