...
| **S7.5-b** | 알람 설정 화면 — `alarms.json` 임계값·확정 시간 편집 | 주석 보존 저장, `AlarmService.ReplaceRules` |
...
2. **S7.5-b — 알람 설정 화면.** `alarms.json` 의 임계값·확정 시간은 아직 편집 수단이 없어
   파일을 직접 고쳐야 하고, 그 경로는 저장 검증을 거치지 않습니다.
   저장은 주석을 보존하는 부분 수정 방식(Newtonsoft `JToken`)으로,
   적용은 `AlarmService.ReplaceRules()` 로 하기로 확정했습니다(2026-08-12).
   I/O Status(램프 12종 + DI 8점)는 **S7.5-a 로 완료**했습니다.
