...
config/
  ...
  control.json     제어 파라미터 (모드별 확정시간·밸브·팬) + 통로 5조. 없어도 기동
docs/
...
**2026-08-19 확정.** 범위는 **영점 교정 · 수동 조작 · Homing/Alarm Reset** 셋입니다.
제어 파라미터 조정은 `control.json` 이 생겼으므로 이제 가능하지만 이번 범위에서는 뺍니다.

| 결정 | 내용 |
|---|---|
| 수동 조작 이탈 | **화면을 떠나면 자동 파킹**(밸브 닫고 팬 정지) + 수동 모드인 동안 **하단 배너에 상시 표시** |
| 지령 우선순위 | `Automatic(0) < Manual(1) < Interlock(2)` — 이미 있음. 인터록이 항상 수동을 이긴다 |
| 영점 저장 | `DeviceMapDocumentEditor` 를 그대로 쓴다(`devices[].offset`) |
| 제외 | 통신 진단(I/O 화면에 이미 있음), 고속 로거(S8과 묶음), 파라미터 조정 |
